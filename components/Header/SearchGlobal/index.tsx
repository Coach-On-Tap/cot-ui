    ...
    <>
      ...
      >
        <Search
          ...
          classButton={`${visible ? "max-lg:visible max-lg:opacity-100" : ""}`}
          value={search}
        ...
        <div
          className={`absolute top-[calc(100%+0.625rem)] left-0 z-100 w-106.5 p-3 rounded-4xl bg-b-surface2 border-1 border-s-subtle shadow-dropdown transition-all max-[1179px]:w-99.5 max-lg:right-0 max-lg:w-auto ${
            visibleResult ? "visible opacity-100" : "invisible opacity-0"
          }`}
        >
          <div className="mb-3">
            <div className="p-3 text-body-2 text-t-secondary">Best match</div>
            <div className="">
              {bestMatch.map((product: any) => (
                <Product value={product} key={product.id} />
              ))}
            </div>
          </div>
          <div className="">
            <div className="p-3 text-body-2 text-t-secondary">Suggestions</div>
            <div className="">
